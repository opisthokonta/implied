...
It is also possible to convert odds to probabilities in the case of <i>multiple winning outcomes</i>, for example if you have the odds for finishing top 3 in a league. In such case the probabilities should sum to 3 instead of 1. The details are explained towards the end of this document.
...
The Odds-Only-Equal-Profitability-Confidence (OO-EPC), method is based on the idea that extreme odds (i.e. favorites and longshots) should be considered less certain than more moderate odds, because fewer bets will be placed on them. This method scale the inverse odds in proportion to their standard errors (i.e. their precision), where the standard error for extreme odds are greater because fewer bets are placed on them. 

The standard error for the inverse odds <i>r<sub>i</sub></i> on outcome <i>i</i> is

SE(<i>r<sub>i</sub></i>) = sqrt((<i>r<sub>i</sub></i> * (1 - <i>r<sub>i</sub></i>)) / <i>r<sub>i</sub></i>)

The inverted odds are then scaled to get the proper probabilities  

<i>p<sub>i</sub></i> = <i>r<sub>i</sub></i> - (z*SE(<i>r<sub>i</sub></i>))

where 

z = (sum(<i>r<sub>i</sub></i>) - t) / sum(SE(<i>r<sub>i</sub></i>))

where t is the target probability, which is usually 1, but see the section on "Multiple winning outcomes".

The method was developed by Kaito Goto and originally made available in a Python package (https://github.com/gotoConversion/goto_conversion/), and  later presented in an article (Goto <i>et al.</i>, 2026) 

```{r goto}
# Balanced books
res10 <- implied_probabilities(my_odds, method = 'ooepc')
...
* Kaito Goto, Naoya Takeishi, Takehisa Yairi (2026) Forecast Sports Outcomes under Efficient Market Hypothesis: Theoretical and Experimental Analysis of Odds-Only and Generalised Linear Models [Link](https://doi.org/10.48550/arXiv.2604.17194)
