...
## The GOTO Conversion method

The GOTO Conversion method is based on the assumption that extreme odds (i.e. favorites and longshots) should be considered to less certain than more moderate odds. This method scale the inverse odds in proportion to their standard errors (i.e. their precision), where the standard error for extreme odds are greater because fewer bets are placed on them. 
...
The method was developed by Kaito Goto and originally made available in a Python package (https://github.com/gotoConversion/goto_conversion/).

```{r goto}
# Balanced books
res10 <- implied_probabilities(my_odds, method = 'goto')
